...
world.afterEvents.playerPlaceBlock.subscribe(({ player, block }) => {
    if (block.permutation.type.id !== "minecraft:chest")
        return;
...
world.beforeEvents.playerInteractWithBlock.subscribe(ev => {
    if (ev.block.permutation.type.id !== "minecraft:chest")
        return;
...
world.beforeEvents.explosion.subscribe(ev => {
    ...
    ev.setImpactedBlocks(impactedBlocks.filter(block => !chestOwners.has(keyFor(block.location))));
});
//# sourceMappingURL=lockableChests.js.map