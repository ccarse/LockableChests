...
world.afterEvents.playerPlaceBlock.subscribe(({ player, block }) => {
  if (block.permutation.type.id !== BlockTypes.chest.id) return;
  ...
  if (!owner && below.type.id === BlockTypes.cobblestone.id) {
...
world.beforeEvents.playerInteractWithBlock.subscribe(ev => {
  if (ev.block.permutation.type.id !== BlockTypes.chest.id) return;
